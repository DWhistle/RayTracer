...
double	len_circle(t_vec point, t_obj2 *sphere);
double	len_cone(t_vec point, t_obj2 *cone);
double	len_cylinder(t_vec point, t_obj2 *cylinder);
double	len_plane(t_vec point, __global t_obj2 *plane);
double			update_r(double r, t_obj2 new_obj, t_vec point, t_scene objs);
t_vec			get_normal(t_vec point, __global  t_obj2 obj);
t_point_data	crate_point_data(t_vec norm,
				t_obj2 *obj, t_vec point, t_vec color);
t_point_data	raymarching(t_scene objs, t_vec vec,
				t_accuracy accuracy, t_vec point, __global  t_obj2 *obj);
t_vec			rand_vec(t_vec vec);
int				is_light(t_scene scene, int ind);
t_point_data	path_tracing(t_scene scene, t_vec vec,
t_accuracy accuracy, t_vec point, __global  t_obj2 *objs);
...
t_vec	ray_render(t_scene scene, t_vec point, t_accuracy accuracy, __global  t_obj2 *objs);
t_vec	antialiasing(t_scene scene, double x, double y, t_accuracy accuracy, __global t_obj2 *objs);
...
double	len_plane(t_vec point, t_obj2 *plane)
{
...
double			update_r(double r, t_obj2 new_obj, t_vec point, t_scene objs)
{
	...
	if (new_obj.type == SPHERE)
		...
		len = len_cone(point, &new_obj);
	if (r == -1 || len < r)
		r = len;
	return (r);
}

t_vec			get_normal(t_vec point, t_obj2 obj)
{
	t_obj2		cylinder;
	double		k;
	t_vec		vec;
	...
	if (obj.type == SPHERE)
		return (vec_norm(vec_sub(point, (obj).point)));
							...
							t_accuracy accuracy, t_vec point, __global  t_obj2 *obj)
{
			...
			vec_len(vec_sub(point, next_point)) < accuracy.max_dist)
	{
		...
		while (counter--)
		{
			...
			obj[counter]), obj + counter, next_point, new_vec0()));
		}
							...
							t_accuracy accuracy, t_vec point, __global  t_obj2 *objs)
{
	...
	while (accuracy.depth_pt > depth_pt++ && point_data2.obj)
	{
		...
		point_data2 = raymarching(scene, rand_vec(norm), accuracy, point, objs);
	}
	return (point_data);
}
...
t_vec	ray_render(t_scene scene, t_vec point, t_accuracy accuracy, __global  t_obj2 *objs)
{
	...
	point_data = path_tracing(scene, vec, accuracy, scene.cam, objs);
	if (point_data.obj)
	{
		points[0] = point_data;
		depth_ref = 0;
...
t_vec	antialiasing(t_scene scene, double x, double y, t_accuracy accuracy, __global t_obj2 *objs)
{
	...
	while (x + d_x < x + 1)
	{
		...
		while (y + d_y < y + 1)
		{
			color_1 = vec_sum(color_1, ray_render(scene, new_vec3((x + d_x), (y + d_y), MAX_Z), accuracy, objs));
							...
							__global int *h)
{
	...
	accuracy->rpp = convert_int(half_sqrt((float)accuracy->rpp));
	scene->lights = lights;
	...
	if (color.arr[2] > 255)
		color.arr[2] = 255;
	pixel[x + *w * y] = convert_int(color.arr[0]) * 256 * 256 + convert_int(color.arr[1]) * 256 + convert_int(color.arr[2]);
}
