...
t_restriction	*get_restriction(t_json *j, int *ns_p)
{
	...
	if (!(r = ft_memalloc(sizeof(t_restriction) * res)))
		return (NULL);
	n = res;
	*ns_p = res;
	while (n--)
	{
		...
		if (!res)
		{
			r[n].dislocation = get_vec(query_attribute(j, "dislocation", &res).json_value);
			if (res)
				r[n].dislocation = new_vec0();
			r[n].norm = vec_norm(get_vec(query_attribute(j, "norm", &res).json_value));
			if (res)
				r[n].norm = new_vec0();
		}
	}
