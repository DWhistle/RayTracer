...
/*   Updated: 2019/08/08 20:24:27 by hgreenfe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    get_rot(t_json *obj, t_scene *scene)
{
    ...
    scene->rot_x = create_quat(new_vec3(1, 0, 0), acos(vec_dotvec(new_vec3(0, 0, 1), vec_norm(new_vec3(0, vec.arr[1], vec.arr[2])))) * signy);
    scene->rot = create_quat(new_vec3(0, 1, 0), acos(vec_dotvec(new_vec3(0, 0, 1), vec_norm(new_vec3(vec.arr[0], 0, vec.arr[2])))) * signx);
}
